...
comictime = {"c84": 1376064000, "c85": 1388246400, "c86": 1408032000, "c87": 1419696000, "c88": 1407945600,
             "c89": 1451318400, "c90": 1470931200, "c91": 1482940800}
...
def count(cur):
    ...
    while i <= n:
        cur.execute(
            'SELECT count(*),sum(favorited),sum(ratings) FROM ehproject.ehdata2 where %s < posted and posted < %s and elanguage = "korean";' % (
...
def kanc(cur):
    ...
    while s < e:
        l = []
        cur.execute(
            'select count(*),sum(Favorited),sum(ratings) from ehproject.ehdata_doujinshi where %s < posted and posted < %s and parody = "touhou project" and elanguage = "japanese" and filecount < 100' % (
                s, s + m))
        sql = cur.fetchone()
        for data in sql:
            str = data.__str__()
...
def touc(cur):
    ...
    while s < e:
        l = []
        cur.execute(
            'select count(*),sum(Favorited),sum(ratings) from ehproject.ehdata_doujinshi where %s < posted and posted < %s and parody = "touhou project" and filecount < 100' % (
                s, s + m))
        sql = cur.fetchone()
        for data in sql:
            str = data.__str__()

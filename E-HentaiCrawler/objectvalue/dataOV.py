...
class dataOV(object):
    def __init__(self, gid=0, token='', archiver_key='', title='', title_jpn='',
        ...
        self.__SQL_FIELD = {'gid': self.__gid, 'token': self.__token, "favorited": self.__favorited,
                            "ratings": self.__ratings, "archiver_key": self.__archiver_key,
                            "title": self.__title, "title_jpn": self.__title_jpn, "elanguage": self.__elanguage,
                            "category": self.__category, "thumb": self.__thumb,
                            "uploader": self.__uploader, "posted": self.__posted, "filecount": self.__filecount,
                            "filesize": self.__filesize, "expunged": self.__expunged,
    ...
    def __str__(self):
        NUMBER_FIELD = ['gid', 'posted', 'favcount', 'ratings', 'filecount', 'filesize', 'rating', 'torrentcount',
                        "favorited"]
        datateble = "ehtest"
        attrs = []
        values = []
...
if __name__ == "__main__":
    ...
    data = {'favorited': 3, 'ratings': 7, 'elanguage': 'Thai ',
            'title_jpn': '(C91) [GRINP (ねことうふ)] おかしいお姉ちゃん (君の名は。) [タイ翻訳]'}
    ov.update(favorited=data['favorited'], ratings=data['ratings'], elanguage=data['elanguage'],
